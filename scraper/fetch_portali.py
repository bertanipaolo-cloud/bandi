...
PAUSA = 1.0
TENTATIVI = 2
# 45 secondi per tentativo, per due tentativi, per 60 portali fa nel caso
# pessimo un'ora e mezza: il 17/08/2026 questo passo si e' mangiato tutti i
# 30 minuti di budget del job e ha fatto saltare tutto il resto. Il timeout
# per singola richiesta scende, e sopra c'e' comunque il budget complessivo.
TIMEOUT = 20
# Tetto di tempo per l'intero giro di portali. Scaduto il tempo si smette di
# interrogarne di nuovi e si salva quello che si e' raccolto: meglio dieci
# portali buoni che un passo ucciso a meta' senza risultati.
BUDGET_MINUTI = 10
# Un portale che non risponde costa TIMEOUT x TENTATIVI di attesa pura. Nel run
# del 24/08/2026 venti host su ventotto erano irraggiungibili e da soli hanno
# consumato quasi tutto il tempo del passo. Il registro si ricorda quindi chi
# fallisce: dopo FALLIMENTI_PRIMA_DI_SALTARE volte di fila il portale viene
# saltato, e riprovato solo ogni GIORNI_DI_QUARANTENA giorni. Non si butta via
# — un ente puo' tornare online — ma smette di costare tempo a ogni giro.
FALLIMENTI_PRIMA_DI_SALTARE = 3
GIORNI_DI_QUARANTENA = 30
...
def _in_quarantena(voce, oggi):
    """Vero se il portale ha fallito troppe volte e non e' ancora ora di riprovare."""
    if (voce.get("falliti_di_fila") or 0) < FALLIMENTI_PRIMA_DI_SALTARE:
        return False
    ultimo = voce.get("ultimo_fallimento")
    if not ultimo:
        return False
    try:
        quando = date.fromisoformat(ultimo)
    except ValueError:
        return False
    return (oggi - quando).days < GIORNI_DI_QUARANTENA


def _scarica(url, tentativi=None):
    tentativi = tentativi or TENTATIVI
...
def main():
    ...
    ap.add_argument("--budget-minuti", type=float, default=BUDGET_MINUTI,
                    help=f"tetto di tempo per l'intero giro (default {BUDGET_MINUTI})")
    ...
    # Col budget di tempo non si arriva sempre in fondo alla lista: se l'ordine
    # fosse fisso, gli ultimi portali non verrebbero interrogati mai. Si parte
    # quindi da quelli visti meno di recente (i mai visti per primi), cosi' su
    # piu' run la copertura ruota su tutto il registro. A parita', chi ha una
    # storia di fallimenti va in fondo.
    portali.sort(key=lambda v: (v.get("falliti_di_fila") or 0, v.get("visto") or ""))

    oggi = datetime.now().date()
    in_quarantena = []
    da_interrogare = []
    for voce in portali:
        if _in_quarantena(voce, oggi):
            in_quarantena.append(voce)
        else:
            da_interrogare.append(voce)
    if in_quarantena:
        print(f"[portali] {len(in_quarantena)} portali in quarantena "
              f"(falliti {FALLIMENTI_PRIMA_DI_SALTARE} volte di fila, "
              f"si riprovano dopo {GIORNI_DI_QUARANTENA} giorni)", file=sys.stderr)
    ...
    limite = args.budget_minuti * 60
    for voce in da_interrogare[:args.max_portali]:
        if time.monotonic() - inizio > limite:
            scaduto = True
            print(f"[portali] budget di {args.budget_minuti:g} minuti esaurito: "
                  f"mi fermo a {interrogati} portali su "
                  f"{min(len(da_interrogare), args.max_portali)}", file=sys.stderr)
            break
        base = voce["base"]
        url = urllib.parse.urljoin(base, PERCORSO_AVVISI)
        interrogati += 1
        # Chi ha gia' fallito merita un solo tentativo, non due: e' meta' del
        # tempo buttato su un host che con ogni probabilita' e' ancora giu'.
        tentativi = 1 if (voce.get("falliti_di_fila") or 0) else TENTATIVI
        try:
            html = _scarica(url, tentativi=tentativi)
        except Exception as e:  # noqa: BLE001 - un portale giu' non ferma il run
            falliti.append((base, f"{type(e).__name__}: {e}"))
            voce["falliti_di_fila"] = (voce.get("falliti_di_fila") or 0) + 1
            voce["ultimo_fallimento"] = oggi.isoformat()
            continue
        trovati = analizza(html, base, ente_ripiego=(voce.get("enti") or [None])[0])
        record.extend(trovati)
        voce["visto"] = oggi.isoformat()
        voce["ultimi_avvisi"] = len(trovati)
        voce["falliti_di_fila"] = 0
        voce.pop("ultimo_fallimento", None)
        time.sleep(PAUSA)
    ...
    coda = " (budget scaduto)" if scaduto else ""
    print(f"[portali] {len(record)} avvisi da {interrogati} portali{coda} "
